...
*/
public class frmPenalidade extends javax.swing.JFrame {
	...
	public frmPenalidade(ArrayList<Individuo> individuosSelecionados, String diretorio, String identificacao) {
		super();
	...
	private DefaultCategoryDataset obtemDataset() {
		...
		for (Individuo ind : individuosSelecionados) {
			if (ind.getSelecionado()) {
				...
				dataset.addValue(ind.getMediaPenalidade(), "Indivíduo selecionado", ind.getGeracao());
			}
		}	
		...
		for (int i = 1; i <= Estrategia.max_gera; i++) {
			...
			for (Individuo ind : individuosSelecionados) {
				if (ind.getGeracao() == i && !ind.getSelecionado()) {					
					//TODO: Ajustar aqui para ficar parametrizado
					dataset.addValue(ind.getMediaPenalidade(), "Indivíduo " + cont, ind.getGeracao());
					cont++;
				}
			}
		}	
	...
	private JFreeChart createChart() {
		...
		JFreeChart chart = ChartFactory.createLineChart(
				...
				"Qtde de penalidades", //x-axis label
				dataset, //data
		...
		chart.setTitle(new TextTitle("Penalidades", new Font("Arial", Font.BOLD, 16)));
