...
*/
public class frmObjetivosGeracoes extends javax.swing.JFrame {
	...
	public frmObjetivosGeracoes(ArrayList<Individuo> individuosSelecionados, String objetivo, Boolean colorir, String diretorio, String identificacao) {
		super();
	...
	private DefaultCategoryDataset obtemDataset() {
		...
		for (Individuo ind : individuosSelecionados) {
			if (ind.getSelecionado()) {
				dataset.addValue(ind.fInad.get(objetivo), "Indivíduo Selecionado ", ind.getGeracao());
			}
		}
		...
		for (int i = 1; i <= Estrategia.max_gera; i++) {
			...
			for (Individuo ind : individuosSelecionados) {
				if (ind.getGeracao() == i && !ind.getSelecionado()) {					
					dataset.addValue(ind.fInad.get(objetivo), "Indivíduo " + cont, ind.getGeracao());
					cont++;
				}
			}
		}
	...
	private JFreeChart createChart() {
		...
		JFreeChart chart = ChartFactory.createLineChart(
				"Objetivo " + objetivo + " nas gerações" + (colorir ? " (colorido)" : ""), //title
				"Gerações", //x-axis category
				objetivo, //x-axis label
				dataset, //data
		...
		chart.setTitle(new TextTitle("Objetivo " + objetivo + " nas gerações" + (colorir ? " (colorido)" : ""), new Font("Arial", Font.BOLD, 16)));
		...
		for (int i = 1; i <= Estrategia.max_gera; i++) {
			...
			if (!colorir) renderer.setSeriesPaint(i, Color.blue);
