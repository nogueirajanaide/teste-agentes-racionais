...
*/
public class frmEpisodiosProblemaGeracoesCT extends javax.swing.JFrame {
	...
	public frmEpisodiosProblemaGeracoesCT(ArrayList<Individuo> individuosSelecionados, String objetivo, String diretorio, String identificacao) {
		super();
	...
	private DefaultCategoryDataset obtemDataset() {
		...
		for (Individuo ind : individuosSelecionados) {	
			...
			if (ind.getSelecionado()) {
				...
				dataset.addValue(media/Estrategia.repeticao, "Indivíduo selecionado", ind.getGeracao());
				System.out.println(ind.getGeracao() + " " + media/Estrategia.repeticao);
			}
		}
		...
		for (int i = 1; i <= Estrategia.max_gera; i++) {
			...
			for (Individuo ind : individuosSelecionados) {
				...
				if (ind.getGeracao() == i && !ind.getSelecionado()) {					
					...
					dataset.addValue(media/Estrategia.repeticao, "Indivíduo " + cont, ind.getGeracao());
					System.out.println(ind.getGeracao() + " " + cont + " "+ media/Estrategia.repeticao);
					cont++;
				}
			}
		}	
	...
	private JFreeChart createChart() {
		...
		JFreeChart chart = ChartFactory.createLineChart(
				"Episódio problema - Objetivo " + objetivo + " por caso de teste", //title
		...
		chart.setTitle(new TextTitle("Episódio problema - Objetivo " + objetivo + " por caso de teste", new Font("Arial", Font.BOLD, 16)));
