...
*/
public class frmBoxPlotUtilidadeGeracoes extends javax.swing.JFrame {
	...
	public frmBoxPlotUtilidadeGeracoes(ArrayList<Individuo> individuosSelecionados, String diretorio, String identificacao) {
		super();
	...
	private BoxAndWhiskerCategoryDataset obtemDataset() {
		...
		for (int i = 1; i <= Estrategia.max_gera; i++) {
			...
			for (Individuo ind : individuosSelecionados) {
				if (ind.getGeracao() == i) {
					values.add(ind.getAvaliacao());
					System.out.print(ind.getAvaliacao() + "  ");
				}
			}
			dataset.add(values, "Geração", i);
			System.out.println("Qtde: " + values.size());
		}
		return dataset;
	}
	...
	private JFreeChart createChart() {
        ...
        final JFreeChart chart = new JFreeChart(
	            "Box-plot de utilidade nas gerações",
	            new Font("SansSerif", Font.BOLD, 14),
	            plot,
	            false
	        );     
		
		chart.setTitle(new TextTitle("Box-plot de utilidade nas gerações", new Font("Arial", Font.BOLD, 16)));
