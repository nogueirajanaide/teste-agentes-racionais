...
*/
public class frmUtilidadeGeracoes extends javax.swing.JFrame {
	...
	public frmUtilidadeGeracoes(ArrayList<Individuo> individuosSelecionados, String diretorio, String identificacao) {
		super();
	...
	private void initGUI() {
		try {
			...
			TableLayout thisLayout = new TableLayout(new double[][] {{23.0, 747.0, TableLayout.FILL}, {20.0, 399.0, 29.0, 15.0, TableLayout.FILL}});
	...
	private DefaultCategoryDataset obtemDataset() {
		...
		for (Individuo ind : individuosSelecionados) {
			if (ind.getSelecionado()) {
				dataset.addValue(ind.getAvaliacao(), "Indivíduo Selecionado", ind.getGeracao());
			}
		}	
		...
		for (int i = 1; i <= Estrategia.max_gera; i++) {
			...
			for (Individuo ind : individuosSelecionados) {
				if (ind.getGeracao() == i && !ind.getSelecionado()) {					
					dataset.addValue(ind.getAvaliacao(), "Indivíduo " + cont, ind.getGeracao());
					cont++;
				}
			}
		}		
	...
	private JFreeChart createChart() {
		...
		chart.setTitle(new TextTitle("Fitness obtido nas gerações", new Font("Arial", Font.BOLD, 16)));
